...
import { getSimilarProjects, projectIsAfterPhotosOnly, type Project, type Room } from "@/lib/realisations";
...
function flatPhotos(
  ...
  beforeLabel: string,
  afterLabel: string
): { photo: Room["before"][number]; label: string }[] {
  return [
    ...room.before.map((photo) => ({ photo, label: beforeLabel })),
    ...room.after.map((photo) => ({ photo, label: afterLabel })),
  ];
}
...
export function ProjectDetailClient({ project }: { project: Project }) {
  ...
  const waHref = useMemo(() => {
    const text = pp.waInterest.replace("{title}", project.title);
    return buildWhatsAppUrl(text, WHATSAPP_E164);
  }, [project.title, pp.waInterest]);

  const afterPhotosOnly = projectIsAfterPhotosOnly(project);
    ...
    <div className="bg-brand-white pb-20">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-start">
          <div>
            <nav className="text-sm text-brand-muted">
              ...
              {" > "}
              <Link href={localizedPath(locale, "/realisations")} className="hover:text-brand-black/80">
                {pp.breadcrumbRealisations}
              </Link>
              {" > "}
              ...
              <button
                ...
                className={cn(
                  ...
                  active
                    ...
                    : "border-transparent text-brand-black hover:text-brand-black/80"
                )}
              >
      ...
      <section className="mx-auto max-w-7xl px-4">
        <div className="rounded-xl bg-brand-black px-6 py-12 text-center text-white md:px-12">
          <h2 className="font-display text-3xl uppercase sm:text-4xl">{pp.ctaTitle}</h2>
          <p className="mx-auto mt-3 max-w-lg text-sm text-white/75">{pp.ctaLead}</p>
          <a
            href={waHref}
            target="_blank"
...
function RoomSection({ room, projectTitle, photosNote }: { room: Room; projectTitle: string; photosNote?: string }) {
  ...
  const flat = useMemo(() => flatPhotos(room, pp.before, pp.after), [room, pp.before, pp.after]);
    ...
    return (
      <section id={roomScrollId(room)} className="scroll-mt-28">
        ...
        <LightboxGallery
          ...
          altPrefix={`${projectTitle} — ${room.name}`}
          layout="carousel"
        />
      </section>
    );
      ...
      {mode === "compare" && before && after && (
        <div className="mt-6 space-y-4">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <p className="mb-2 text-xs font-heading font-bold uppercase text-brand-black">{pp.before}</p>
                ...
                {room.before.map((ph, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      bi === i ? "ring-brand-black" : "ring-transparent hover:ring-black/40"
                    )}
                  >
                    <Image
                      src={ph.url}
            ...
            <div>
              <p className="mb-2 text-xs font-heading font-bold uppercase text-brand-muted">{pp.after}</p>
              <div className="flex flex-wrap gap-2">
                {room.after.map((ph, i) => (
                  <button
                    key={ph.id}
                    ...
                    className={cn(
                      "relative h-16 w-24 overflow-hidden rounded-md ring-2 transition-shadow",
                      ai === i ? "ring-brand-black" : "ring-transparent hover:ring-black/40"
                    )}
                  >
                    <Image
                      src={ph.url}
      ...
      {mode === "side" && (
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <div className="space-y-3">
            <p className="text-xs font-heading font-bold uppercase text-brand-black">{pp.before}</p>
            {room.before.map((ph, i) => (
              <button
                key={ph.id}
                type="button"
              ...
              >
                <span className="absolute left-2 top-2 z-[1] rounded bg-brand-black px-2 py-0.5 text-xs font-heading font-bold uppercase text-white">
                  {pp.before}
                </span>
                <div className="relative aspect-[4/3] w-full">
                  <Image
                    src={ph.url}
                    alt={ph.caption ?? pp.before}
                    fill
                    className="object-cover"
          ...
          <div className="space-y-3">
            <p className="text-xs font-heading font-bold uppercase text-brand-muted">{pp.after}</p>
            {room.after.map((ph, i) => (
              <button
                key={ph.id}
                type="button"
              ...
              >
                <span className="absolute left-2 top-2 z-[1] rounded bg-brand-black px-2 py-0.5 text-xs font-heading font-bold uppercase text-white ring-1 ring-black/10">
                  {pp.after}
                </span>
                <div className="relative aspect-[4/3] w-full">
                  <Image
                    src={ph.url}
                    alt={ph.caption ?? pp.after}
                    fill
                    className="object-cover"
                    sizes="(max-width:768px) 100vw, 50vw"
                    loading="lazy"
